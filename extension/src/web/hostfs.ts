...
export class HostFS
  implements
    FileSystemProvider,
    /*FileSearchProvider, TextSearchProvider,*/ Disposable {
  ...
  constructor(peer: any) {
    ...
    this.disposable = Disposable.from(
      workspace.registerFileSystemProvider(HostFS.scheme, this, {
  ...
  stat(uri: Uri): Thenable<FileStat> {
    ...
    return new Promise<FileStat>(
      (resolve: (value: FileStat) => void, reject: (reason?: any) => void) => {
  ...
  readDirectory(uri: Uri): Thenable<[string, FileType][]> {
    ...
    return new Promise<[string, FileType][]>(
      async (
        resolve: (value: [string, FileType][]) => void,
  ...
  delete(uri: Uri, options: { recursive: boolean }): Thenable<void> {
    return new Promise<void>(
      async (
        ...
        // parent.size -= 1;
        this._fireSoon({ type: FileChangeType.Changed, uri: dirname }, {
          uri,
